...
// ------------------------------------------------------------------
// Full schema – **idempotent** (CREATE IF NOT EXISTS, ALTER IF NOT)
// ------------------------------------------------------------------
const schema = `
-- ============================================================
-- 1️⃣  Core tables (unchanged – never drop or rename)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS users (
  ...
  password_hash TEXT,                     -- used by current auth code / tests
  password VARCHAR(255),                  -- new column for compatibility (optional)
  role VARCHAR(50) DEFAULT 'warehouse_staff',   -- original default kept (tests expect this)
...
-- ============================================================
-- 3️⃣  Offline‑first extensions (already existed but kept)
-- ============================================================
...
async function migrate() {
    try {
        ...
        // Split on semicolons, keep statements that are not empty
        ...
        for (const [i, stmt] of statements.entries()) {
            ...
            try {
                // Neon expects a query string; we cast to any to satisfy the type signature
                await (sql as any)(stmt);
                // tiny pause to avoid overwhelming the DB
                await new Promise((r) => setTimeout(r, 150));
            } catch (err: any) {
                // “already exists” errors are expected for idempotent runs
